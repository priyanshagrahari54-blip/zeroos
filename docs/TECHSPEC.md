...
`DISPLAY_PRESENT` (ID 26) — pixel-mapping scanout submit; ABI feature bits 7
(`ZEROOS_ABI_FEATURE_DISPLAY`) and 8 (`ZEROOS_ABI_FEATURE_PRESENT`). Any new
syscall ID must be mirrored in `kernel/syscall.h`,
...
- `ZEROOS_SYS_DISPLAY_PRESENT` (ID 26, feature bit 8) is the pixel-mapping
